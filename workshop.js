...
// Single authoritative frontend state machine for join, polling, phases, moderator controls, and return-to-join.
(function(){
'use strict';
...
const GUIDE={
  1:{say:'Please read the starting record and complete your first response individually before discussing it.',do:'Let everyone work independently. Submitting does not move the group. Click Next phase when the room is ready.'},
  2:{say:'Now compare your starting points and identify what you most need to know.',do:'Clarify instructions only. Do not suggest what they should notice. Click Next phase when ready.'},
  3:{say:'Choose the two evidence areas you think would be most useful to this decision.',do:'Do not recommend evidence packets. Let the group choose. Click Next phase when ready.'},
  4:{say:'Review what you selected and discuss what it answers and what remains uncertain.',do:'Keep attention on what the evidence actually supports. Click Next phase when ready.'},
  5:{say:'Discuss your recommendation, then every participant should submit an individual choice.',do:'Do not force consensus. Preserve disagreement. Click Next phase when ready.'},
  6:{say:'Additional information is now available. Consider what, if anything, it changes.',do:'Do not imply that participants should change position. Click Next phase when ready.'},
  7:{say:'Build the strongest case you can for the alternative shown on your screen.',do:'Encourage a serious case, not a straw man. Click Next phase when ready.'},
  8:{say:'Make your final judgment and identify what would make it workable or cause you to reconsider.',do:'Preserve dissent. Do not push for agreement. Click Next phase when ready.'},
  9:{say:'Now step out of the scenario and evaluate the experience as educators.',do:'Focus on authenticity, reasoning opportunities, scaffolds and observability. When finished, return to the main Zoom room.'}
};

const $=s=>document.querySelector(s);
const $$=s=>Array.from(document.querySelectorAll(s));
const esc=s=>String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));
const SESSION_KEY='spark_live_session_v1';
const POLL_MS=C.POLL_MS||2500;
...
function api(action,payload={}){
  return new Promise((resolve,reject)=>{
    if(!backendAvailable()) return reject(new Error('Backend relay is not available. The Apps Script backend must be deployed for anonymous access.'));
    let done=false;
    const timeout=setTimeout(()=>{if(!done){done=true;reject(new Error('Backend did not respond within 12 seconds.'));}},12000);
    try{
      google.script.run
        .withSuccessHandler(r=>{if(done)return;done=true;clearTimeout(timeout);resolve(r);})
...
function showJoin(prefill=true){stopPolling();renderedStage=0;$('#workshopView')?.classList.add('hidden');$('#waitingView')?.classList.add('hidden');$('#facilitatorView')?.classList.add('hidden');$('#joinView')?.classList.remove('hidden');$('#changeSessionBtn')?.classList.add('hidden');
  if(prefill&&session.name)$('#participantName').value=session.name;else $('#participantName').value='';
  $('#entryRole').value='participant';$('#moderatorCodeWrap').classList.add('hidden');$('#moderatorCode').value='';selectedGroup='';renderGroupCards();
  $('#roleBadge').textContent='Teacher / Player';setConnection(backendAvailable()?'Backend ready':'Backend unavailable',backendAvailable());
}

async function joinWorkshop(){
  clearError();
  const name=$('#participantName').value.trim();
  const role=$('#entryRole').value;
  const key=role==='moderator'?$('#moderatorCode').value.trim():'';
  ...
  const btn=$('#joinBtn');btn.disabled=true;btn.textContent='Connecting…';
  ...
  try{
    ...
    await enterWorkshop();
  }catch(e){showError(e.message||String(e));setConnection('Backend error',false);}
...
async function enterWorkshop(){
  $('#joinView').classList.add('hidden');$('#workshopView').classList.remove('hidden');$('#changeSessionBtn').classList.remove('hidden');
...
async function fetchState(forceRender){
  try{
    ...
    if(forceRender||renderedStage!==state.stage||oldStage!==state.stage)renderStage();
    else refreshLiveElements();
    updateTimer();
  }catch(e){setConnection('Connection problem',false);console.error(e);}
}
...
function updateTimer(){const el=$('#timer');if(!el)return;if(!state.deadline){el.textContent='--:--';return;}const ms=Math.max(0,Number(state.deadline)-(Date.now()+clockOffset));const sec=Math.ceil(ms/1000);el.textContent=`${Math.floor(sec/60)}:${String(sec%60).padStart(2,'0')}`;}

function renderStage(){
  renderedStage=Math.max(1,Math.min(9,Number(state.stage)||1));
  const st=STAGES[renderedStage-1];
  $('#stageKicker').textContent=`Phase ${st.id} of 9`;
  $('#stageTitle').textContent=st.title;
  $('#stageContent').innerHTML=stageHtml(st.id);
  renderModeratorStrip();
  wireStage();
  if(session.role==='moderator')disableParticipantInputs();
  updateTimer();
}

function renderModeratorStrip(){
  const old=$('#moderatorStrip');if(old)old.remove();
  if(session.role!=='moderator')return;
  const host=$('#workshopView');
  const strip=document.createElement('section');strip.id='moderatorStrip';strip.className='card compact';strip.style.marginBottom='12px';
  const next=state.stage<9?'<button id="modNext" class="primary">Next phase →</button>':'<span class="badge">Breakout final phase</span>';
  const g=GUIDE[state.stage]||GUIDE[9];
  strip.innerHTML=`<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><strong style="margin-right:auto">Moderator · ${ICON[session.group]} ${session.group}</strong><span id="moderatorProgress" class="badge secondary">Phase ${state.stage} of 9</span><button id="modPlus" class="secondary-btn">+1 min</button>${next}<button id="modGuide" class="secondary-btn">Guide</button></div><div id="modGuidePanel" class="${guideOpen?'':'hidden'}" style="margin-top:10px"><p><strong>SAY:</strong> ${esc(g.say)}</p><p><strong>DO:</strong> ${esc(g.do)}</p></div>`;
...
function currentGroupChoice(){const r6=voteResult(state.votes?.s6),r5=voteResult(state.votes?.s5);return r6.choice||r5.choice||'';}
...
function stageHtml(n){
  ...
  if(n===5){if(state.votes?.s5?.[session.id])return `<section class="card stage-card"><h3>Preliminary group recommendation</h3>${voteSummary('s5')}</section>`+submittedCard(5);return `<section class="card stage-card"><h3>Preliminary group recommendation</h3><p>Discuss first. Then every participant submits an individual choice. Consensus or a strict majority (&gt;50%) is a group result; plurality alone is not.</p>${decisionChoices('s5decision')}${confidence('s5conf')}<label>What evidence matters most to your recommendation?<textarea id="s5why"></textarea></label><label>What important uncertainty remains?<textarea id="s5unknown"></textarea></label><button class="primary submit-vote" data-stage="5">Submit recommendation</button>${voteSummary('s5')}</section>`;}
  if(n===6){if(state.votes?.s6?.[session.id])return `<section class="card stage-card"><h3>Evolving information</h3>${updateFacts()}${voteSummary('s6')}</section>`+submittedCard(6);return `<section class="card stage-card"><h3>Evolving information</h3>${updateFacts()}<p>Consider whether this information actually changes your earlier reasoning. Maintaining your position can be as defensible as revising it.</p>${decisionChoices('s6decision')}${confidence('s6conf')}<label>Why does this information change or not change your recommendation?<textarea id="s6why"></textarea></label><label>What uncertainty matters most now?<textarea id="s6unknown"></textarea></label><button class="primary submit-vote" data-stage="6">Submit updated recommendation</button>${voteSummary('s6')}</section>`;}
  if(n===7){const base=currentGroupChoice();const target=challengeFor(base);return `<section class="card stage-card"><h3>Perspective Challenge</h3>${base?`<div class="callout"><strong>Current group direction:</strong> ${esc(base)}<br><strong>Your challenge:</strong> Build the strongest defensible case for <em>${esc(target)}</em>.</div>`:`<div class="callout"><strong>No consensus or strict majority is available.</strong> Choose a defensible alternative position to examine rather than treating a plurality as the group decision.</div><label>Alternative position to test<select id="manualChallenge"><option value="">Choose…</option>${DECISIONS.map(x=>`<option>${esc(x)}</option>`).join('')}</select></label>`}<label>Strongest case for the alternative<textarea id="challengeCase"></textarea></label><label>What would have to be true for that alternative to be defensible?<textarea id="challengeConditions"></textarea></label><label>What evidence would discriminate between the competing positions?<textarea id="challengeEvidence"></textarea></label><button class="primary group-submit" data-key="stage7">Submit perspective challenge</button></section>`;}
  if(n===8){if(state.votes?.s8?.[session.id])return `<section class="card stage-card"><h3>Final decision</h3>${voteSummary('s8')}</section>`+submittedCard(8);return `<section class="card stage-card"><h3>Final decision, conditions & reconsideration</h3><p>Make your final judgment. A change is not inherently better than maintaining a position.</p>${decisionChoices('s8decision')}${confidence('s8conf')}<label>Conditions or safeguards needed, if any<textarea id="s8conditions"></textarea></label><label>What should be monitored after the decision?<textarea id="s8monitor"></textarea></label><label>What future evidence would make you reopen this decision?<textarea id="s8reopen"></textarea></label><button class="primary submit-vote" data-stage="8">Submit final decision</button>${voteSummary('s8')}</section>`;}
  return `<section class="card stage-card"><h3>Educator reflection & co-design</h3><p>Step out of the scenario and evaluate the experience as an educator.</p><label>What felt authentic or inauthentic?<textarea id="authenticity"></textarea></label><label>Where did consequential reasoning actually occur?<textarea id="reasoningMoments"></textarea></label><label>Which scaffolds helped, and which felt leading or unnecessary?<textarea id="scaffolds"></textarea></label><label>What learner reasoning could you realistically observe or collect?<textarea id="observable"></textarea></label><label>What should we change before classroom use?<textarea id="changes"></textarea></label><button class="primary group-submit" data-key="stage9">Submit reflection</button><div class="callout" style="margin-top:16px"><strong>Breakout activity complete.</strong> Please return to the main Zoom room when your moderator asks you to.</div></section>`;
}
function updateFacts(){return `<div class="facts">${(D.stage6Update||[]).map(x=>`<div class="fact">${esc(x)}</div>`).join('')}</div>`;}
function voteSummary(key){const r=voteResult(state.votes?.[key]||{});if(!r.total)return '<div class="card compact vote-summary"><p class="muted">No participant choices submitted yet.</p></div>';return `<div class="card compact vote-summary"><div class="eyebrow">Current group result</div><p><strong>${esc(r.label)}${r.choice?`: ${esc(r.choice)}`:''}</strong>${r.choice?` (${r.max} of ${r.total})`:''}</p></div>`;}
...
function wireStage(){
  ...
  $$('.submit-vote').forEach(b=>b.onclick=async()=>{
    const st=Number(b.dataset.stage);const choice=$(`input[name=s${st}decision]:checked`)?.value;if(!choice)return alert('Choose a recommendation first.');
    const conf=Number($(`#s${st}conf`)?.value||50);b.disabled=true;b.textContent='Submitting…';
    try{
      let r=await api('vote',{group:session.group,id:session.id,name:session.name,stage:st,choice,confidence:conf});if(!r||r.ok===false)throw new Error((r&&r.error)||'Vote failed');
      if(st===1)r=await api('submit',{group:session.group,id:session.id,key:`s1_${session.id}`,value:{name:session.name,factors:$('#s1factors')?.value||'',unknown:$('#s1unknown')?.value||''}});
      if(st===5)r=await api('submit',{group:session.group,id:session.id,key:`s5_${session.id}`,value:{name:session.name,why:$('#s5why')?.value||'',unknown:$('#s5unknown')?.value||''}});
      if(st===6)r=await api('submit',{group:session.group,id:session.id,key:`s6_${session.id}`,value:{name:session.name,why:$('#s6why')?.value||'',unknown:$('#s6unknown')?.value||''}});
      if(st===8)r=await api('submit',{group:session.group,id:session.id,key:`s8_${session.id}`,value:{name:session.name,conditions:$('#s8conditions')?.value||'',monitor:$('#s8monitor')?.value||'',reopen:$('#s8reopen')?.value||''}});
      if(r&&r.ok===false)throw new Error(r.error||'Submission failed');await fetchState(true);
    }catch(e){alert(e.message||e);b.disabled=false;b.textContent='Submit response';}
  });
  $$('.group-submit').forEach(b=>b.onclick=async()=>{b.disabled=true;b.textContent='Submitting…';try{const value={...collectScope(b.closest('section')),submittedBy:session.name};const r=await api('submit',{group:session.group,id:session.id,key:b.dataset.key,value});if(!r||r.ok===false)throw new Error((r&&r.error)||'Submission failed');b.textContent='Submitted ✓';await fetchState(false);}catch(e){alert(e.message||e);b.disabled=false;b.textContent='Submit group response';}});
  const eb=$('#selectEvidenceBtn');if(eb)eb.onclick=async()=>{const ids=$$('input[name=evidence]:checked').map(x=>x.value);if(ids.length!==2)return alert('Choose exactly two evidence packets.');eb.disabled=true;eb.textContent='Submitting…';try{let r=await api('selectEvidence',{group:session.group,id:session.id,ids});if(!r||r.ok===false)throw new Error((r&&r.error)||'Evidence selection failed');r=await api('submit',{group:session.group,id:session.id,key:'stage3',value:{ids,why:$('#evidenceWhy').value,questions:$('#evidenceQuestions').value,submittedBy:session.name}});if(!r||r.ok===false)throw new Error((r&&r.error)||'Submission failed');eb.textContent='Submitted ✓';await fetchState(false);}catch(e){alert(e.message||e);eb.disabled=false;eb.textContent='Submit evidence choice';}};
}

function bindStaticControls(){
  renderGroupCards();
  $('#entryRole').onchange=()=>$('#moderatorCodeWrap').classList.toggle('hidden',$('#entryRole').value!=='moderator');
  $('#joinBtn').onclick=joinWorkshop;
  $('#changeSessionBtn').onclick=()=>{clearSession();showJoin(false);};
}

function boot(){
  bindStaticControls();
  loadSession();
  if(new URLSearchParams(location.search).get('fresh')==='1'){clearSession();showJoin(false);return;}
  if(session.id)resumeSession();else showJoin(false);
}
