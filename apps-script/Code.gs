/** SPARK Workshop 1 backend v2.1: static relay + optimized Google Sheets store */
const ACCESS_KEY=['Go','Bears'].join('');
const GROUP_IDS=['Owl','Fox','Raven','Dolphin','Octopus'];
const STAGE_MINUTES=[4,6,5,8,7,5,6,8,18];
const GH_BASE='https://paruchuri9-sys.github.io/SPARK-Workshop1/';
const SHEETS={participants:'ParticipantsV2',responses:'ResponsesV2',votes:'VotesV2',events:'EventsV2',group:'GroupDataV2'};
const SETUP_CACHE_KEY='spark_setup_v21';

function doGet(e){
  const p=(e&&e.parameter)||{};
  if(String(p.bridge||'')==='1'){
    return HtmlService.createHtmlOutput(relayHtml_())
      .setTitle('SPARK API Relay')
      .addMetaTag('viewport','width=device-width, initial-scale=1')
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  }
  const target=String(p.view||'')==='dashboard' ? GH_BASE+'dashboard.html' : GH_BASE;
  return HtmlService.createHtmlOutput('<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="refresh" content="0;url='+target+'"></head><body><script>location.replace('+JSON.stringify(target)+')</script><p>Opening SPARK…</p></body></html>')
    .setTitle('SPARK Workshop 1')
    .addMetaTag('viewport','width=device-width, initial-scale=1');
}

function relayHtml_(){
  return '<!doctype html><html><head><meta charset="utf-8"></head><body>'
    +'<script>(function(){function send(m){parent.postMessage(m,"*")}window.addEventListener("message",function(ev){var d=ev.data||{};if(d.type!=="spark-api"||!d.id)return;google.script.run.withSuccessHandler(function(r){send({type:"spark-api-result",id:d.id,ok:true,result:r})}).withFailureHandler(function(err){send({type:"spark-api-result",id:d.id,ok:false,error:(err&&err.message)||String(err)})}).api(d.action,d.payload||{})});send({type:"spark-relay-ready"})})();</script>'
    +'</body></html>';
}
...
function stateForGroup_(g){
  const cache=CacheService.getScriptCache();
  const key='spark_state_v21_'+g;
  const hit=cache.get(key);
  if(hit){
    try{const x=JSON.parse(hit);x.serverNow=Date.now();return x;}catch(e){}
  }
  ...
  cache.put(key,JSON.stringify(state),1);
  return state;
}

function invalidateGroupCache_(g){CacheService.getScriptCache().remove('spark_state_v21_'+g);}

function dashboardData_(){
  const ss=SpreadsheetApp.getActiveSpreadsheet(),people=readParticipants_(),gd=readGroupData_(),votes=readAllVotes_(),groups={};
  GROUP_IDS.forEach(g=>{const p=people.filter(x=>x.group===g),raw=gd[g]||{};groups[g]={state:{started:!!raw._started,stage:Math.max(1,Math.min(9,Number(raw._stage||1))),deadline:raw._deadline==null?null:Number(raw._deadline)},participants:p.filter(x=>x.role!=='moderator'),moderators:p.filter(x=>x.role==='moderator'),selectedEvidence:raw.selectedEvidence||[],votes:votes[g]||{},groupData:stripControl_(raw)};});
