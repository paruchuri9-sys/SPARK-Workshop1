/**
 * SPARK Workshop 1 - Apps Script backend v0.3
...
function handle_(q){
  ...
  if(a==="event"){logEvent_(q.group||"",q.participant||"",q.event||"",q.stage||"",q);return {ok:true};}
  if(a==="submit"){
  ...
  if(a==="ready"){logEvent_(q.group,q.participant,"ready",q.stage,{});return {ok:true};}
  if(a==="facilitator"){
    Object.keys(q.patch||{}).forEach(k=>setConfig_(k,String(q.patch[k]??"")));
    if(q.patch&&q.patch.stage){
      const s=Number(q.patch.stage),mins=STAGE_MINUTES[s-1]||5;
      setConfig_("prompt","");
...
function stateForGroup_(g){
  ...
  return {
    stage:all.stage,deadline:all.deadline,prompt:all.prompt,
    selectedEvidence:all.selectedEvidenceByGroup[g]||[],
    votes:all.votes[g]||{},groupData:gd,
...
function allState_(){
  ...
  const selected={};Object.keys(gd).forEach(g=>selected[g]=gd[g].selectedEvidence||[]);
  return {stage:Number(cfg.stage||1),deadline:Number(cfg.deadline||0)||null,prompt:cfg.prompt||"",selectedEvidenceByGroup:selected,votes,groupData:gd,participants,ready};
}
...
function readVotes_(){
  ...
  v.slice(1).forEach(r=>{const g=String(r[1]),s="s"+r[2],p=String(r[3]);o[g]=o[g]||{};o[g][s]=o[g][s]||{};o[g][s][p]={choice:r[4],confidence:Number(r[5]),ts:new Date(r[0]).getTime()};});
  return o;
}
function readReady_(){
  const v=SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Events").getDataRange().getValues(),o={};
  v.slice(1).forEach(r=>{if(r[3]==="ready"){const g=String(r[1]);o[g]=o[g]||[];if(!o[g].includes(String(r[2])))o[g].push(String(r[2]));}});
  return o;
}
...
function getConfig_(){
  ...
  v.slice(1).forEach(r=>{if(r[0])o[r[0]]=r[1];});return o;
}
